    ...
    return(
        <div className = "homework border-1">
            <div className = "homework_title border-btm">
                <div className = "title ">
                    <b>[일반과제] {this.props.title}</b><this.checkLimitDate update = {this.props.updated} limitdate = {this.props.limitdate} />
                    <div className = "tasks">
                        <ul className = "ul-nolist-inline">
                            <li><Link to = {`homeworkmodify?p=${this.props.course}&id=${this.props.id}`} className = "btn btn_classtask">수정</Link></li>
                            <li><Link to = {`homeworkevaluation?p=${this.props.course}&id=${this.props.id}`}  className = "btn btn_classtask">평가</Link></li>
                        </ul>
                    </div>
                </div>
            </div>
